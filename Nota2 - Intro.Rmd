...
Vemos que el valor en la segunda pocision cambió a `4`.
...
El proceso para acceder a elemento de listas en un poco diferente. Si intentamos hacer lo mismo, obtendremos otra lista de un solo elemento que contiene al dato buscado, en lugar de el dato buscado por si solo.
...
Por úlitmo veamos la función `unlist()`, la cual convierte una lista a un vector.
...
Sin embargo la forma propia de acceder este valor es con un espacio en blanco en la sección de las filas y el indice de la columna despues de la coma.
```{r}
#La primera columna
arreglo[,1]
```

Por otro lado, si queremos solo la primera fila, escribimos la coma pero dejamos en blanco lo que viene desupués.
...
table(vector_numerico2) #util para graficar

#Usando vectores como conjuntos
union(vector_numerico, vector_numerico2) #Union
intersect(vector_numerico, vector_numerico2) #Interseccion
setdiff(vector_numerico, vector_numerico2) #Diferencia (elementos del primero que no estan en el segundo)
setequal(vector_numerico, vector_numerico2) #Si son iguales o no (sin importar el orden)

```
La función `table()` recibe un vector y cuenta las repeticiones de cada valor. Regresa un vector con nombres donde el nombre indica el valor que se está contando y el valor del vector las veces que se repite. En este caso tenemos que el `2` se repite 3 veces, le `4` 2 veces y el `5` una vez.
...
La función `sqrt()` regresa la raiz cuadrada del número ingresado.
